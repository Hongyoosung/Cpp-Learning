...
Hello World
```
반복에 사용될 변수 i를 초기식에서 선언 후 0으로 초기화, 조건식 검사, 문장 실행 후 i를 1증가. 해당 과정이 반복되어 i가 5가 되는 순간 문장을 실행하지 않고 반복문을 빠져나온다.
따라서 위 for 문은 i가 0, 1, 2, 3, 4 일 때, 총 5번의 문장 반복을 수행한다.
...
21 22 23 24 25
```
이중 반복문은 반복문의 실행 문장에 또 다른 반복문을 구현한 것이다. 바깥의 반복문이 한 번 실행될 때, 안의 반복문은 모든 반복을 마치고 빠져나오고, 바깥 반복문의 2번째 실행이
시작되는 것이다. 따라서 위의 경우, 실행 결과로 미루어 볼 때, 바깥 반복문은 행을, 안의 반복문은 열을 담당한다. 이러한 이중 반복문의 활용을 통해 2차원 매트릭스를 구현할 수 있다.


## 0부터 입력받은 수 까지의 구구단 출력하기
```c++
#include <iostream>
using namespace std;
